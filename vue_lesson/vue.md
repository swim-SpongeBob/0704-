...
其实 ref 也可以直接写在组件上，那么获取的就是该组件的实例，也就是说可以获取组件内的所有内容，进而也就可以实现父子组件之间的交互

##### 组件内的 this

组件内使用 this 其实就是想要使用该组件下的 data props methods computed 自定义事件 ref 等等等，那么组件内我们通常使用的 this 就指该组件内的实例(VueComponent)
