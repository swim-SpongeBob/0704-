<template>
  <div>
    ...
    <transition name="fade1" mode="out-in">
      ...
      <!-- 上述写法的简写方式 -->
      <!-- 不需要使用v-if 控制，需要使用 :key 控制 -->
      <button @click="state = !state" :key="state">
        {{ state ? "off" : "on" }}
      </button>
      <!-- 当你点击按钮的时候实现的动画效果，按钮先渐渐消失然后再渐渐出现 -->
      <!-- 按照之前的过渡效果去设置的话，两个元素会同时执行动画 如果希望一个元素先执行效果，另外一个在执行的话，需要给 transition 加 mode 属性 -->
    </transition>
    <h3>列表过渡 列表的进入和离开</h3>
    ...
    <transition-group class="list" name="list" tag="ul">
      <li v-for="num in listNum" :key="num">{{ num }}</li>
    </transition-group>
  </div>
</template>
